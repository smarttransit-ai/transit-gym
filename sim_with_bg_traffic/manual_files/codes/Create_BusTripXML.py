...
# testing
if os.path.exists('E:/SUMO/RUIXIAO/newChattanooganet/SUMO_simulation/0BusLines.trips.xml'):
    os.remove("E:/SUMO/RUIXIAO/newChattanooganet/SUMO_simulation/0BusLines.trips.xml")

# read from stopsinf
data = pd.read_excel("E:/SUMO/RUIXIAO/newChattanooganet/Data/busstops.xlsx",index_col = 'ID')
...
f = open("E:/SUMO/RUIXIAO/newChattanooganet/SUMO_simulation/0BusLines.trips.xml", "x")

# write first line
f.write("<routes>\n")
vtype = ['DieselBus','HybridBus','ElectricBus']
for tripid, df in trip.items():
    df['stop_id'] = df.index
    # add one element as bus type           
    tripid=tripid+(random.choices(vtype, weights=(70, 20, 10), k=1)[0],) 
    f.write('\t<trip id="Route'+str(tripid[2])+"_trip"+str(tripid[1])+
            "_"+str(tripid[4])+'" line="Route'+str(tripid[2])+
            "_"+tripid[3]+"_block"+str(tripid[5])+"_trip"+str(tripid[1])+'" type="'
